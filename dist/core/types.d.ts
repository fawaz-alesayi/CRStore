...
type Schema<T> = T extends {
    TYPE: infer U;
} ? U : unknown;
type Operation<T extends any[], R = void, S = any> = (db: Kysely<S>, ...args: T) => Promise<R>;
type Selectable<T> = {
    execute(): Promise<T>;
    compile(): CompiledQuery;
    toOperationNode(): SelectQueryNode;
};
type Actions<Schema> = Record<string, (db: Kysely<Schema>, ...args: any[]) => Promise<any>>;
type Bound<Actions> = {
    [K in keyof Actions]: Actions[K] extends (db: Kysely<any>, ...args: infer A) => infer R ? (...args: A) => Promise<Awaited<R>> : never;
};
type Node = SelectQueryNode | TableNode | ColumnNode | ReferenceNode | RawNode | AggregateFunctionNode | AliasNode | SelectAllNode;
type View<Schema, Type, Deps extends any[] = []> = (db: Kysely<Schema>, ..._: Deps) => Selectable<Type[]>;
type Update<S> = {
    update<T extends any[], R>(operation?: Operation<T, R, S>, ...args: T): Promise<R>;
};
type Context<Schema> = {
    update<T extends any[], R>(operation: Operation<T, R, Schema>, ...args: T): Promise<R>;
    refresh<T = unknown>(query: CompiledQuery, id: {
        queryId: string;
    }): Promise<T[]>;
    subscribe(tables: string[], callback: () => any): () => void;
    connection: Promise<Kysely<Schema>>;
};
type CoreStore<S> = <T, A extends Actions<S>, D extends any[]>(view: View<S, T, D>, actions?: A, dependencies?: D) => PromiseLike<T[]> & Bound<A> & Update<S> & {
    subscribe: (fn: (value: T[]) => void) => () => void;
    bind: (parameters: D | ((set: (updated: D) => void) => (() => void) | void)) => void;
};
...
type Pull = ((input: {
    ...
    onData: (changes: EncodedChanges) => any;
}) => {
    unsubscribe(): void;
}) | undefined;
interface Connection<S> extends Kysely<S> {
    selectVersion(): Executable<{
        current: number;
        synced: number;
    }>;
    ...
    applyOperation<T extends any[], R>(operation: Operation<T, R, S>, ...args: T): Executable<{
        result: Awaited<R>;
...
interface CoreDatabase<S> {
    ...
    close(): Promise<void>;
    subscribe(tables: string[], callback: Updater, options?: {
        client: string;
        version: number;
    }): () => void;
}
export type { EncodedChanges, CoreDatabase, Connection, CoreStore, Operation, QueryId, Actions, Context, Updater, Update, Change, Kysely, Schema, Bound, Error, View, Push, Pull, Node, };
